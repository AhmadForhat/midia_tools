    ...
    return (
        <>
            ...
            <body className="contato" style={{marginTop:'50px', display:'flex', height:'400px', flexWrap:'wrap'}}>
                ...
                <informacoes style={{display:'flex',textAlign:'center', flexDirection:'column', width:'30%', minWidth:'200px', margin:'0 auto', height:"100%", marginBottom:'40px'}}>
                <textos>
                ...
                <redes>
                    ...
                    <Input title={'Cadastre-se na nossa Newsletter:'} type={'email'} placeholder={'digite seu email aqui'} valor={email} setValor={setEmail}/>
                    {!news && !loading && !errorNews && <Button click={sendToBackend(state)} small={true} value="click" texto = {'Cadastrar'}/>} 
                    {news && !errorNews && !loading && <h3>Enviado!</h3>}
                    {errorNews && !loading &&(
                    <>
                    <h3>Email já cadastrado!</h3><Button click={sendToBackend(state)} small={true} value="click" texto = {'Cadastrar'}/>
                    </>
                    )
                    }
                    {loading && (
                        <div style={{display:'flex', margin:'0 atuo', marginTop:'40px'}}> 
                            <Spinner />
                        </div>
                    )}
                    </div>
